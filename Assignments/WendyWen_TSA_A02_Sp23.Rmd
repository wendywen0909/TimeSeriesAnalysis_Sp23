...
```{r}
newdata <- energy_data[,c("Month","Total Biomass Energy Production", "Total Renewable Energy Production", "Hydroelectric Power Consumption")]
head(newdata)
```
...
```{r}
#Method 1: using cor()
cor1 <- cor(ts_data[, c('Total Biomass Energy Production', 'Total Renewable Energy Production', 'Hydroelectric Power Consumption')])
cor1
```
```{r}
#Method 2: using rcorr()
library("Hmisc")
cor2 <- rcorr(as.matrix(ts_data[,2:4]),type="pearson")
cor2
```
```{r}
...
pacf_bio <-pacf(ts_data[,1],lag.max = 40)
pacf_bio
```
