...
plot(Time, det[,1], type="l",col="orange", xlab = "Time", ylab= paste0(col_names[1]," [Trillion Btu]"),main=paste0("Detrended ",col_names[1]))
```
...
plt <- ggplot(df_ren) + 
  ...
  labs(title ="Original, Detrended, and Differenced Series of Total Renewable Energy Production") +
...
```
Differencing is more efficient in eliminating the trend.
...
# 
df_ren3 <- df_ren[,2:4]
ts_ren <- ts(df_ren3, start = c(1973,2), frequency = 12)
# Seasonal Mann Kendall test
SMKtest <- SeasonalMannKendall(ts_ren[,1])
print("Results for Seasonal Mann Kendall")
print(summary(SMKtest))

# ADF test
print("Results for ADF test")
print(adf.test(ts_ren[,1],alternative = "stationary"))
```
...
```{r}
ts_ren2 <- as.ts(ts_ren[1:588,])
date2 <- as_date(df_ren[,1], tz = NULL)
date2 <- date2[1:588]
df_ren2 <- cbind(date2, ts_ren2)

ren2_matrix <- matrix(ts_ren2[,1],byrow=FALSE,nrow=12)
ren2_yearly <- colMeans(ren2_matrix)

library(dplyr)  #move this to package chunk later
year <- c(year(first(date2)):year(last(date2)))

df_ren2_yearly <- data.frame(year, ren2_yearly)
```
