...
dec_wind <- decompose(clean_wind_data)
dec_wind2 <- stl(clean_wind_data, s.window = "periodic")
plot(dec_wind2)
# Bars on the right: the degree of zoom in - the longer the bar, the less significant the component

#Ex2: Store the three component in separate vector
tre_wind <- dec_wind$trend
sea_wind <- dec_wind$seasonal
ran_wind <- dec_wind$random

#Ex3: Create one data frame with all three vector and the Date columns from wind_data
df_wind <- data.frame("Date" = wind_data$DATE, tre_wind, sea_wind, ran_wind)
