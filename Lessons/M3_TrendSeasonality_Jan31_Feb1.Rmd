...
ggplot(inflow_data,aes(x = my_date, y = inflow_data[,iHP+1])) +
  geom_line(color = "blue") +
  geom_line(aes(y = linear_trend$residuals), color = "green")
```
...
```{r}
#Use seasonal means model (default: constant) : should use detrend data
iHP = 1

dummies <- seasonaldummy(ts_inflow_data[,iHP])
seas_means <- lm(inflow_data[,iHP+1]~dummies)
summary(seas_means)

beta0_s <- seas_means$coefficients[1]
beta1_s <- seas_means$coefficients[2:12]

inflow_seas_comp <- array(0, nobs)
for (i in 1:nobs){
  inflow_seas_comp[i] = beta0_s + beta1_s %*% dummies[i,]
}
